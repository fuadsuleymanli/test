...
<!-- START BREADCRUMB -->
                ...
                <div class="page-title">
                    <h2><span class="fa fa-users"></span> Active Users <small>{{ count($activeUsers) }} contacts</small></h2>
                </div>
                <!-- END PAGE TITLE -->
                            ...
                            <div class="panel panel-default">
                                <div class="panel-body profile">
                                    <div class="profile-image">
                                        <img src="{{ url('/') }}/guide/photo/{{ $activeUser->photo }}" alt="{{ $activeUser->first_name }} {{ $activeUser->last_name }}"/>
                                    </div>
                                    <div class="profile-data">
                                        <div class="profile-data-name">{{ $activeUser->first_name }} {{ $activeUser->last_name }}</div>
                                        <div class="profile-data-title">
                                          @if($activeUser->user_type == 2)
                                          Tour Guide
                                          @else
                                          Customer
                                          @endif
                                    ...
                                    <div class="profile-controls">
                                        <a href="{{ route('admin.user.userprofile', ['id' => $activeUser->id]) }}" class="profile-control-left"><span class="fa fa-info"></span></a>
                                        <a href="#" class="profile-control-right"><span class="fa fa-phone"></span></a>
                                    </div>
                                </div>
                                <div class="panel-body">
                                    <div class="contact-info">
                                        <p><small>Mobile</small><br/>{{ $activeUser->phone }}</p>
                                        <p><small>Email</small><br/>{{ $activeUser->email }}</p>
                                        <p><small>Address</small><br/>{{ $activeUser->address }}</p>
                                    </div>
                                </div>
                            </div>
                            <!-- END CONTACT ITEM -->
